...
turtles-own [
  ...
  vision-points   ;; the points that this turtle can see in relative to its current position (based on vision)
  metabolism
]
...
to go
  ...
  let clusters dbscan:cluster-by-location turtles 3 6
  let ctr 1
  (foreach clusters (n-of (length clusters) base-colors)
    [ [ x y ] -> let aset turtles with [ member? self x ]
      ask aset
      [ set color y
        ;;set label (word "ID: " who ", Cluster: " ctr ", Wealth: " sugar)
      ]
...
to-report average-wealth
  let total-wealth sum [sugar] of turtles
  let num-turtles count turtles
  if num-turtles > 0 [
    report total-wealth / num-turtles
  ]
  report 0 ;; return 0 if there are no turtles to avoid division by zero
end

to-report total-wealth-of-pop
  let total-wealth-ofpop sum [sugar] of turtles
  report total-wealth-ofpop
end

to-report average-vision
  let total-vision sum [vision] of turtles
  let num-turtles count turtles
  if num-turtles > 0 [
    report total-vision / num-turtles
  ]
  report 0
end

to-report average-metabolism
  let total-metabloism sum [metabolism] of turtles
  let num-turtles count turtles
  if num-turtles > 0 [
    report total-metabloism / num-turtles
  ]
  report 0
end
...
to-report num-clusters
  let clusters dbscan:cluster-by-location turtles 3 6
  report length clusters
end

to-report avg-cluster-size
  let clusters dbscan:cluster-by-location turtles 3 6
  report count turtles / length clusters
end

to-report count-turtles-in-max-sugar-zone
  let max-sugar-patches patches with [max-psugar = 4]
  let turtle-count count turtles-on max-sugar-patches
  report turtle-count
end

to-report turtle-counts-in-sugar-bins
  let min-sugar min [sugar] of turtles
  let max-sugar max [sugar] of turtles

  ;; Calculate the bin size
  let bin-size (max-sugar - min-sugar) / 20

  ;; Initialize a list to store the count of turtles in each bin
  let bin-counts n-values 20 [0]

  ;; Iterate through turtles to assign them to bins
  foreach [sugar] of turtles [current-sugar ->
    let bin floor ((current-sugar - min-sugar) / bin-size)
    ;; Increment the count of turtles in the respective bin
    if bin >= 20 [
      set bin 19 ; Ensure the bin index doesn't exceed the maximum bin index
    ]
    set bin-counts replace-item bin bin-counts (item bin bin-counts + 1)
  ]
...
to-report max-sugar-of-turtles
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot avg-cluster-size"
...
455
NIL
count-turtles-in-max-sugar-zone
17
1
11
...
@#$#@#$#@
<experiments>
  <experiment name="vary_metabolism" repetitions="50" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="200"/>
    <metric>average-vision</metric>
    <metric>average-wealth</metric>
    <metric>total-wealth-of-pop</metric>
    <metric>ripleys-k 10</metric>
    <metric>num-clusters</metric>
    <metric>avg-cluster-size</metric>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="initial-population">
      <value value="200"/>
    </enumeratedValueSet>
    <steppedValueSet variable="mean-metabolism" first="1" step="1" last="8"/>
  </experiment>
  <experiment name="vary_vision" repetitions="50" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="150"/>
    <metric>average-metabolism</metric>
    <metric>average-wealth</metric>
    <metric>total-wealth-of-pop</metric>
    <metric>num-clusters</metric>
    <metric>avg-cluster-size</metric>
    ...
    <steppedValueSet variable="mean-vision" first="1" step="1" last="12"/>
  </experiment>
  <experiment name="vary_sugar" repetitions="50" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="150"/>
    <metric>average-vision</metric>
    ...
    <steppedValueSet variable="sugar-var" first="1" step="1" last="20"/>
  </experiment>
  <experiment name="vary_vision_pop" repetitions="50" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="400"/>
    ...
    <metric>average-wealth</metric>
    <metric>total-wealth-of-pop</metric>
    <metric>count-turtles-in-max-sugar-zone</metric>
    <metric>gini-index</metric>
    <metric>turtle-counts-in-sugar-bins</metric>
    <metric>max-sugar-of-turtles</metric>
    <metric>count turtles</metric>
    <steppedValueSet variable="initial-population" first="50" step="50" last="1250"/>
    <steppedValueSet variable="max-vision" first="0" step="1" last="6"/>
  </experiment>
  <experiment name="vary_pop" repetitions="50" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="150"/>
    <metric>mean [metabolism] of turtles</metric>
